////////////////////////////////////////////////////////////////////////////////
// Filename: fogfireshaderclass.cpp
////////////////////////////////////////////////////////////////////////////////
#include "fogfireshaderclass.h"


FogFireShaderClass::FogFireShaderClass()
{
	m_vertexShader = 0;
	m_pixelShader = 0;
	m_layout = 0;
...
FogFireShaderClass::FogFireShaderClass(const FogFireShaderClass& other)
{
}
...
bool FogFireShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;


	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, hwnd, L"./data/fogfire.vs", L"./data/fogfire.ps");
	if (!result)
	{
		return false;
	}
...
void FogFireShaderClass::Shutdown()
{
...
bool FogFireShaderClass::Render(ID3D11DeviceContext* deviceContext, int vertexCount, int instanceCount, XMMATRIX worldMatrix, XMMATRIX secondWorldMatrix, XMMATRIX  viewMatrix,
	...
	XMFLOAT2 distortion3, float distortionScale, float distortionBias, float fogStart, float fogEnd)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, secondWorldMatrix, viewMatrix, projectionMatrix, fireTexture, noiseTexture, alphaTexture,
		frameTime, scrollSpeeds, scales, distortion1, distortion2, distortion3, distortionScale,
		distortionBias, fogStart, fogEnd);
	if (!result)
	{
		return false;
	}
...
bool FogFireShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* vsFilename, const WCHAR* psFilename)
{
	HRESULT result;
...
void FogFireShaderClass::ShutdownShader()
{
...
void FogFireShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, const WCHAR* shaderFilename)
{
	char* compileErrors;
...
bool FogFireShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX secondWorldMatrix, XMMATRIX  viewMatrix,
	XMMATRIX  projectionMatrix, ID3D11ShaderResourceView* fireTexture,
	...
	float distortionBias, float fogStart, float fogEnd)
{
	HRESULT result;
	...
	secondWorldMatrix = XMMatrixTranspose(secondWorldMatrix);
	...
	dataPtr->secondWorld = secondWorldMatrix;
...
void FogFireShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int vertexCount, int instanceCount)
{
