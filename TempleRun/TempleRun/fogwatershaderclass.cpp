...
FogWaterShaderClass::FogWaterShaderClass()
{
	m_vertexShader = 0;
	m_pixelShader = 0;
	m_layout = 0;
...
FogWaterShaderClass::FogWaterShaderClass(const FogWaterShaderClass& other)
{
}
...
bool FogWaterShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	bool result;

	// Initialize the vertex and pixel shaders.
	result = InitializeShader(device, hwnd, L"./data/fogwater.vs", L"./data/fogwater.ps");
	if (!result)
	{
		return false;
	}
...
void FogWaterShaderClass::Shutdown()
{
...
bool FogWaterShaderClass::Render(ID3D11DeviceContext* deviceContext, int vertexCount, int instanceCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	...
	ID3D11ShaderResourceView* normalTexture, float waterTranslation, float reflectRefractScale, float fogStart, float fogEnd)
{
	bool result;
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, reflectionMatrix, reflectionTexture,
		refractionTexture, normalTexture, waterTranslation, reflectRefractScale, fogStart, fogEnd);
	if (!result)
	{
		return false;
	}
...
bool FogWaterShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, const WCHAR* vsFilename, const WCHAR* psFilename)
{
	HRESULT result;
	...
	polygonLayout[4].AlignedByteOffset = 12;
...
void FogWaterShaderClass::ShutdownShader()
{
...
void FogWaterShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, const WCHAR* shaderFilename)
{
	char* compileErrors;
...
bool FogWaterShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
	XMMATRIX projectionMatrix, XMMATRIX reflectionMatrix,
	...
	float waterTranslation, float reflectRefractScale, float fogStart, float fogEnd)
{
	HRESULT result;
...
void FogWaterShaderClass::RenderShader(ID3D11DeviceContext* deviceContext, int vertexCount, int instanceCount)
{
